...
type JsonCustomConverter<'TSource, 'TTarget, 'TConverter when 'TConverter: (new: unit -> 'TConverter) and 'TConverter :> IMyJsonConvertor<'TSource, 'TTarget>>
    () =
...
type JsonCustomOptionConverter<'TSource, 'TTarget, 'TConverter when 'TConverter: (new: unit -> 'TConverter) and 'TConverter :> IMyJsonConvertor<'TSource, 'TTarget>>
    () =
...
type ListConverter() =
    ...
    override this.CreateConverter(typeToConvert: Type, _options: JsonSerializerOptions) : JsonConverter =
        ...
        let converterType =
            typedefof<ListValueConverter<_>>.MakeGenericType (typArgs)
...
type MapValueConverter<'K, 'V when 'K: comparison>() =
    ...
    override this.Write(writer: Utf8JsonWriter, value: Map<'K, 'V>, options: JsonSerializerOptions) =
        ...
        value
        |> Map.iter (fun k v -> dictionary.Add(k, v))
...
type MapConverter() =
    ...
    override this.CreateConverter(typeToConvert: Type, _options: JsonSerializerOptions) : JsonConverter =
        ...
        let converterType =
            typedefof<MapValueConverter<_, _>>.MakeGenericType (typArgs)
